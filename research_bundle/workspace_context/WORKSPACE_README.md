...
> **RGBIR训练扩展（2026-09-06）**：[对象判别蒸馏三seed执行记录](../08_实验日志/2026-09-06_train_RGBIR对象判别蒸馏三seed扩展/README.md)。首版方法保持冻结，94 GPU4/5/6各一个训练，覆盖paired/weight0×seed0/42/123；原paired42约89/200，暂无最终AP。原生数据流在已测范围固定，seed重复覆盖初始化变化；不能把训练CSV的AP占位0当作结果。
...
- **查方法历史**：[04_方法演化档案/2026-04~06_LADD主线/LADD_EVOLUTION_AND_EXPERIMENTS.md](../04_方法演化档案/2026-04~06_LADD主线/LADD_EVOLUTION_AND_EXPERIMENTS.md) 是最完整的单篇叙事；51 个方法族的机器可读目录见 hub索引文档/catalog（本包未收录：`04_方法演化档案/hub索引文档/catalog/`）。
...
| 服务器 | 角色 | 本地证据位置 |
|---|---|---|
| **94**（10.103.12.94，gpuserver94，8×4090） | **现行主力**：RGB-T 蒸馏（RGBT_campaign）+ SpaceNet6 复现；旧 OGSOD/freqmix 实验在其 `projects/ydf/` | 服务器端为主；SpaceNet6 工程本地副本在 [03/SpaceNet6_OTD_official_reproduction](../03_现行工程/SpaceNet6_OTD_official_reproduction)；freqmix 等记录见 04 方法演化档案（本包未收录：`04_方法演化档案/2026-08_服务器环境总结/`） |
