...
def main(root, existing42):
    root.mkdir(parents=True, exist_ok=False)
    launched = [42]
    def run_dir(seed):
        return existing42 if seed == 42 else root/f'C1_seed{seed}'
    while True:
        states = {s:read(run_dir(s)/'run_final.json') for s in launched}
        failures = {s:v['status'] for s,v in states.items() if v and v['status'] != 'COMPLETED'}
        active = [s for s,v in states.items() if v is None]
        gate = read(existing42/'training/budget_progress.json')
        ...
        status = dict(launched_seeds=launched, pending_seeds=pending, active_seeds=active,
                      ...
                      max_concurrent_runs=2, existing_seed42=str(existing42),
                      recovery_reason='seed0_shared_card_budget_abort; unchanged_training_new_attempt', updated_at=datetime.datetime.now().astimezone().isoformat())
        if failures:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--existing42', required=True, type=Path)
    args = parser.parse_args()
    raise SystemExit(main(args.output, args.existing42))
