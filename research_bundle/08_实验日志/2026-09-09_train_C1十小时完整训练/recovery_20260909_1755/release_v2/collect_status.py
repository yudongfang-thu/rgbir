...
def main():
    result = dict(observed_at=datetime.datetime.now().astimezone().isoformat(),
                  campaign=read(ROOT/'recovery_attempt2/campaign_status.json'),
                  original_campaign=read(ROOT/'attempt1/campaign_status.json'), runs={})
    for seed in (42,0,123):
        run = ROOT/('attempt1' if seed == 42 else 'recovery_attempt2')/f'C1_seed{seed}'
        if not run.exists():
