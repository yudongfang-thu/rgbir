...
def training_completion(cfg, checkpoint, config_path=None):
    ...
    fixed = dict(status='OBJECT_DFL_TRAINING_COMPLETED', scope=SCOPE, endpoint=ENDPOINT,
    ...
    for key in ('arm', 'seed', 'dataset', 'model', 'teacher', 'reference', 'method_identity',
...
def run(args):
    ...
    try:
        ...
        receipt = dict(status='OBJECT_DFL_EVALUATION_COMPLETED', scope=SCOPE, endpoint=ENDPOINT,
            seed=42, arm=cfg['arm'], dataset=cfg['dataset'], method_identity=cfg['method_identity'], single_seed=True, epochs=3,
        ...
        write_new(output/'direction_evaluation_failure.json', dict(status='OBJECT_DFL_EVALUATION_FAILED',
