...
def summarize(rows):
    ...
    for variant in VARIANTS:
        ...
        maxima={k:max([abs(s[k]) for s in sides if k in s] or [0.]) for k in ('mass_closure_error','expectation_closure_error_bin','edge_closure_error')}
        output[variant]=dict(GT_objects=len(group),available_pairs=available,status_counts=counts,
            ...
            recomputed_FP64_vs_cached_FP32_max_abs=max([r.get('recomputed_FP64_vs_cached_FP32_max_abs',0.) for r in group] or [0.]))
    return output
