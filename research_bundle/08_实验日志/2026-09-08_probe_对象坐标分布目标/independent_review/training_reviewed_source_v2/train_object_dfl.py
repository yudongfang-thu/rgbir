"""Warm-start N/L3-DFL/L3-GT on a fixed 2048-image subset; fresh optimizer and EMA."""
import argparse
...
import types
import yaml
from object_dfl_common import ENDPOINT,copy_sources,data_output,explicit,load_config,read,stat,write_new
from object_dfl_common import validate_amp_prior,setup_with_validated_amp
...
def build_private(runtime,criterion,selection,classification,cfg,args):
    ...
    from object_dfl_criterion import make_type
...
def run(args):
    ...
    if not args.canary:
        ...
        if c.get('status')!='OBJECT_DFL_CANARY_COMPLETED' or c.get('arm')!=cfg['arm'] or c.get('successful_updates',0)<24:
    ...
    output.mkdir(parents=True,exist_ok=False);shutil.copyfile(args.config,output/'direction_config.yaml')
    ...
    data_sources += [Path(__file__).with_name('validated_amp_prior.json'),Path(__file__).with_name('validated_amp_prior_config.yaml')]
    ...
    amp_prior=validate_amp_prior(cfg)
    write_new(output/'validated_amp_binding.json',amp_prior)
    ...
    try:
        ...
        def setup():
            ...
            trainer.object_dfl_amp_setup=setup_with_validated_amp(trainer,original_setup,cfg['amp'],amp_prior['actual_amp'],native_trainer_module)
        ...
        result=dict(status='OBJECT_DFL_CANARY_COMPLETED' if args.canary else 'OBJECT_DFL_TRAINING_COMPLETED',
            scope='OBJECT_DFL_FT3_BNFROZEN',method_identity=cfg['method_identity'],single_seed=True,arm=cfg['arm'],seed=42,dataset=cfg['dataset'],
            epochs_configured=3,last_epoch=trainer.epoch+1,endpoint=ENDPOINT,formal_e200_complete=False,
            config_copy=str(output/'direction_config.yaml'),model=cfg['model'],teacher=cfg['teacher'],reference=cfg['reference'],
            ...
            setup_amp=trainer.object_dfl_amp_setup,validated_amp_prior=amp_prior,
        ...
        write_new(output/'direction_failure.json',dict(status='OBJECT_DFL_TRAINING_FAILED',error=repr(error),
