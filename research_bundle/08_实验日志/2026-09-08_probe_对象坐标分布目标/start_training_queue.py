...
review_name='TRAINING_SOURCE_REVIEW.json' if version=='1' else 'TRAINING_SOURCE_REVIEW_v'+version+'.json'
r=json.loads((root/'independent_review'/review_name).read_text(encoding='utf-8'))
...
for reviewed in r['reviewed_source_files']:
 ...
 b=p.read_bytes()
 snapshot=Path(reviewed['snapshot_path'])
 if not snapshot.is_absolute():snapshot=root/snapshot
 if root.resolve() not in snapshot.resolve().parents:raise ValueError('Review snapshot outside evidence entry')
 if b!=snapshot.read_bytes():raise ValueError('Reviewed source changed: '+name)
