"""Deploy new source bytes only. No checkpoint, dataset transfer, or GPU call."""
from pathlib import Path
import base64,io,json,subprocess,sys,tarfile
root=Path(__file__).parent;release=root/'trainer_release';version=sys.argv[1]
if not version.isdigit():raise ValueError('Numeric version')
dest='/mnt/dataset/yudongfang/projects/RGBT_campaign/artifacts/rgbir_object_dfl_screen_20260908/release_v'+version
buf=io.BytesIO()
...
with (root/('TRAINING_DEPLOYMENT_v'+version+'.json')).open('x',encoding='utf-8') as f:json.dump(r,f,indent=2)
