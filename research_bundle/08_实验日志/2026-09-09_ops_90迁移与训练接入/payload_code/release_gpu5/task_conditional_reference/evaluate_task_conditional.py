...
REPO=Path('/mnt/dataX/ydf/projects/RGBT_campaign_90')
...
def main():
    ...
    result['per_class']=[{'class_id':int(cls),'name':model.names[int(cls)],
        'AP50':float(metrics.box.all_ap[i,0]),'AP75':float(metrics.box.all_ap[i,5]),
        'mAP50_95':float(metrics.box.all_ap[i].mean()),
    ...
    emit_bound_run_receipt(run_dir=a.run/'eval_evidence',method_identity=cfg['method_identity'],
        ...
        inputs={'method_id':cfg['method_id'],'arm':completed['arm'],'checkpoint':str(checkpoint),
            'endpoint':'fixed_budget_last_ema','teacher_labels_used_by_kd':completed['arm']!='c_same_modal'})
