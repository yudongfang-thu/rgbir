...
"""
import argparse, importlib.util, json, os, shutil, sys, types
...
def run(args):
    ...
    ready=json.loads(Path(cfg['readiness_receipt']).read_text())
    ...
    def build(bound_cfg, config_path, output, arm=None, max_steps=None, historical=False):
        ...
        for i,p in enumerate(paths):
            ...
            records.append(dict(source=str(p),copy=str(target),source_metadata=source_record(target)))
        write_new(Path(output)/'fastpath_launch_manifest.json',dict(
            ...
            candidate_source_metadata=source_record(plan['candidate_source']),original_reference=str(ref),
            readiness_scope='90 deployment only; no inherited 94 admission',
            new_implementation_admission=plan['source_bindings'],sources=records,
    ...
    try:
        ...
        receipt=dict(status='FAST_C1_E200_COMPLETED',implementation_id=plan['implementation_id'],
            ...
            candidate_source=plan['candidate_source'],candidate_source_metadata=source_record(plan['candidate_source']),
        ...
        runtime.legacy.emit_bound_run_receipt(run_dir=args.output/'fastpath_run_evidence',
            ...
            inputs=dict(implementation_id=plan['implementation_id'],fresh_common_initialization=True,
                initial_weights=cfg['model'],candidate_source_metadata=source_record(plan['candidate_source']),old_run_continuation=False))
