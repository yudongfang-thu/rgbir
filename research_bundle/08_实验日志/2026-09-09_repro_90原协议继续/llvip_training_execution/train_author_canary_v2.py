...
def main():
    ...
    try:
        ...
        # Author main converts CLI Path defaults before serializing opt.yaml.
        # Direct train(...) needs the same type normalization, with unchanged values.
        for key, value in vars(opt).items():
            if isinstance(value, Path):
                setattr(opt, key, str(value))
        yaml.safe_dump(vars(opt))
