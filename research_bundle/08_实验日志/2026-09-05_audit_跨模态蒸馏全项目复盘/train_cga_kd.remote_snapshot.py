...
class InvariantHeads(nn.Module):
    ...
    def __init__(self, channels, student_channels):
        super().__init__()
        mid = 128
        self.g = nn.Sequential(nn.Conv2d(channels, mid, 1), nn.GELU(), nn.Conv2d(mid, 64, 1))
        self.h = nn.Sequential(nn.Conv2d(channels, mid, 1), nn.GELU(), nn.Conv2d(mid, 64, 1))
        self.g_proj = nn.Linear(64, student_channels)
        self.disc = nn.Sequential(nn.Linear(64 + student_channels, 64), nn.GELU(), nn.Linear(64, 1))

    def embed(self, feats):
        g = self.g(feats).mean(dim=(2, 3))
        h = self.h(feats).mean(dim=(2, 3))
        return g, h
...
class CGACriterion:
    ...
    def _ensure_heads(self, teacher_feats, student_feats):
        if self.arm != "invariant" or self.aux is not None:
            return
        channels = teacher_feats[LEVEL_ALIGN[0]].shape[1]
        student_channels = student_feats[LEVEL_ALIGN[0]].shape[1]
        self.aux = InvariantHeads(channels, student_channels).to(teacher_feats[0].device)
    ...
    def __call__(self, prediction, batch):
        ...
        if self.arm == "geom":
            ...
            g_p, h_p = self.aux.embed(teacher["feats"][LEVEL_ALIGN[0]])
            g_s, h_s = self.aux.embed(teacher_shuf["feats"][LEVEL_ALIGN[0]])
            ...
            for li in LEVEL_ALIGN:
                ...
                gf = self.aux.g_proj(self.aux.g(teacher["feats"][li]).mean(dim=(2, 3)))
