...
def opportunity_summary(rows, mask, same_anchor=False):
    ...
    teacher = [object_view(r, "T42", same_anchor) for r in selected]
    pair = np.array([r.get("paired_gt_iou") is not None and r["paired_gt_iou"] >= PAIR_IOU for r in selected])
    ...
    out["paired_gt"] = {"n": int(pair.sum()), "fraction_all_gt": ratio(pair.sum(), n)}
...
def probe_analysis(rows, features, logits, groups, output):
    ...
    for donor in ("T42", "N0"):
        if donor + "_logits" in blocks:
            plans["N_logits+" + donor + "_logits"] = ["N_logits", donor + "_logits"]
...
def dfl_analysis(rows, logits, groups, output):
    ...
    result = {"status": "COMPLETED", "bins": 16, "temperature": TEMPERATURE,
              "support": "All four GT distances in [0,15], no clamping; exact 15 is a point mass at final bin.",
...
def report_markdown(result):
    ...
    lines += ["", "以上 T 特征输入是在 probe 评价阶段使用教师的双模态诊断；它衡量该线性函数族是否可解码额外信息，不能叫作 RGB-only 推理提升或 KD 增益。shuffled 在各 split 内按固定 seed 全局置换，不条件化于标签；同模态 N0 仅在已导出时报告。N_region_logits 是共同区域内容代理，不是冻结 C1 算子的复刻。", "",
              "## 同 anchor 定位分布（dev）", ""]
