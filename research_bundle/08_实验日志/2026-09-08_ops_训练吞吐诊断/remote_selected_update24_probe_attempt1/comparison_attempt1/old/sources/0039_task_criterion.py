...
class TaskCriterion(legacy.EvidenceCriterion):
    ...
    def __call__(self, prediction, batch):
        ...
        with torch.no_grad():
            ...
            content = legacy.raw_prediction(self.teacher(batch['content_img'])) if self.arm == 'c_shuffled' else None
        strides = tuple(int(s) for s in self.trainer.model.stride)
        if self.arm == 'c_shuffled':
            c_loss, c_stats = object_evidence_content_loss(student, teacher, reference, batch,
                content_teacher=content, strides=strides, config=self.evidence_cfg,
                arm='paired', seed=self.cfg['seed'] + self.calls)
        elif self.arm == 'c_same_modal':
            c_loss, c_stats = same_modal_evidence_loss(student, teacher, reference, batch,
                strides=strides, config=self.evidence_cfg, seed=self.cfg['seed'] + self.calls)
        else:
        ...
        wl = 0.0 if self.arm in ('n', 'c', 'c_shuffled', 'c_same_modal') else float(self.cfg['localization_coefficient'])
