...
def short_completion(run,cfg):
    ...
    if (result.get('status')!='SHORT_SCREEN_TRAINING_COMPLETED' or result.get('scope')!='SHORT_SCREEN'
        or result.get('single_seed') is not True or result.get('last_epoch')!=20 or result.get('epochs_configured')!=20
        ...
        raise ValueError('Completed independent E20 SHORT_SCREEN receipt required')
...
def run(args):
    ...
    try:
        ...
        write_new(output/'short_evaluation_receipt.json',dict(status='SHORT_SCREEN_EVALUATION_COMPLETED',
            ...
            epochs=20,independent_lr_horizon=20,formal_e200_complete=False,formal_paper_gain_claim=False,
