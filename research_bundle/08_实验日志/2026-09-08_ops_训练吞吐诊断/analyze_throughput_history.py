...
OUT=HERE/'history_evidence_v2';OUT.mkdir(exist_ok=False)
...
for name in ('N0','N42','N123','C0','C42','C123','R0','R42','R123','S42','M42'):
    ...
    raw=[{k.strip():v.strip() for k,v in row.items() if k is not None and k.strip() in ('epoch','time')} for row in raw]
