...
class PolicyTests(unittest.TestCase):
    ...
    def test_same_computation_profile_requires_actual_binding_and_sufficient_peak(self):
        profile = dict(schema=dispatch.PROFILE_SCHEMA, status='COMPLETED', measurement_valid=True,
                       ...
                       companion_measurement={'verified':True},
    ...
    def test_single_measurement_does_not_authorize_second_formal_train(self):
        r=reservation(False);r['profiled_second_train']=False
        state=usage([0]);state['effective_cuda_processes'][0]=1;state['actual_cuda_pids'][0]=1
        state['effective_vram_mib'][0]=7500;state['actual_vram_mib'][0]=7500
        g=rows(2);g[0].update(memory_free_mib=17064,memory_used_mib=7500,all_process_pids=[7])
        requested=dict(job('train',False),formal=True,requires_profile='/synthetic/profile')
        self.assertEqual(dispatch.admission_candidates(state,g,requested,r,4)[0],[1])

    def test_companion_is_profiled_canary_and_requires_one_actual_task(self):
        requested=dict(job('canary',False),require_project_companion=True,requires_profile='/synthetic/profile')
        dispatch.validate_job(requested)
        with self.assertRaises(ValueError):
            dispatch.validate_job(dict(job(),require_project_companion=True))
        state=usage([0]);state['effective_cuda_processes'][0]=1
        self.assertEqual(dispatch.admission_candidates(state,rows(2),requested,reservation(False),4)[0],[])
        state['actual_cuda_pids'][0]=1
        self.assertEqual(dispatch.admission_candidates(state,rows(2),requested,reservation(False),4)[0],[0])

    def test_evaluation_profile_has_separate_bootstrap_and_binding_path(self):
        requested=dict(job('evaluation_profile'),kind='eval')
        self.assertTrue(dispatch.measured_reservation(requested)['bootstrap'])
        self.assertEqual(dispatch.compute_path('evaluation_profile'),dispatch.compute_path('evaluation'))
        profile=dict(schema=dispatch.PROFILE_SCHEMA,status='COMPLETED',measurement_valid=True,
            stage='evaluation_profile',profile_key='synthetic-C1-b32',evaluation_profile_binding='fixture',
            profile_identity={'actual':'bound'},resources={'per_gpu_peak_vram_mib':{'0':7000},'peak_rss_mib':30000})
        fake=types.ModuleType('evaluator_profile')
        fake.validate_evaluation_profile_binding=lambda path,cfg,root:{'actual':'bound'}
        with tempfile.TemporaryDirectory() as folder:
            cfg=Path(folder)/'cfg.yaml';cfg.write_text('dataset: dronevehicle\n')
            future=dict(job('evaluation',False),kind='eval',config=str(cfg),requires_profile='fixture')
            with patch.object(dispatch,'read_json',return_value=profile),patch.dict(sys.modules,{'evaluator_profile':fake}):
                self.assertTrue(dispatch.measured_reservation(future)['measured'])
