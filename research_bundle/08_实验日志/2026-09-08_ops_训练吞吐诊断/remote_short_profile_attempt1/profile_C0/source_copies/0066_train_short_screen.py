"""DRAFT independent E20 entry; explicit reviewed admission required to execute."""
import argparse
import json
...
def run(args):
    ...
    # formal=False does not enforce E200/readiness; this entry owns all E20
    ...
    try:
        ...
        trainer.train()
        if trainer.epoch+1!=20:raise RuntimeError('SHORT_SCREEN did not complete exactly20 epochs')
        ...
        result=dict(status='SHORT_SCREEN_TRAINING_COMPLETED',scope='SHORT_SCREEN',single_seed=True,
            arm=cfg['arm'],seed=42,dataset='dronevehicle',epochs_configured=20,last_epoch=20,endpoint=ENDPOINT,
            independent_lr_horizon=20,not_e200_prefix=True,formal_e200_complete=False,
