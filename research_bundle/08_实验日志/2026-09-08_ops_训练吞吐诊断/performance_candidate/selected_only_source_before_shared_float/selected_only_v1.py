...
class SelectedOnlyAPI:
    ...
    def build(self, student, teacher, reference, batch, *, strides=(8, 16, 32),
        ...
        learning = self.first_pass(student, teacher, reference, batch, **kwargs)
        ...
        for region in learning.regions:
            ...
            sval, sv = self.original_pool(student['scores'][bi, :, start:end], fg, bg,
                learning.config.minimum_foreground, learning.config.minimum_background)
            with torch.no_grad():
                tval, tv = self.original_pool(teacher['scores'][bi, :, start:end], tfg, tbg,
                    learning.config.minimum_foreground, learning.config.minimum_background)
