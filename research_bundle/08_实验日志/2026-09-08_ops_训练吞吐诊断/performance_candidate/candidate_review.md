...
## 追加：02:14 修复版代码复核，真实回执待到达

重新只读核对 `capture_real_batch.py` 14434 bytes、`benchmark_candidate.py` 15123 bytes（两者本地 mtime 2026-09-08 02:14:07）以及相邻 `../run_real_probe.py`。上面的初审原文保留，不回写旧判断或旧回执。

- **第 1 项在 raw-score 范围内已修复。** capture 独立调用 `autograd.grad(kd, s['scores'], retain_graph=True)`，显式检查梯度存在、有限、L2>0；失败时不替换固定 batch。该耗时单列，并记录包含/扣除此探针的 pipeline span。由此可排除“native 总梯度掩盖 KD raw-score 全零”，但仍未测 KD 独立共享参数梯度，也没有新/旧实现的 optimizer/EMA/AMP 更新比较。待真实 receipt 的实际范数验证后才能声称这些 batch 通过。
- **第 2 项代码修复完成。** `timed(..., track_peak=True)` 由外层拥有峰值区间，内层池化使用 `track_peak=False`，仍同步测 wall time但不 reset、不输出伪内层峰值。外层 replay 峰值区间不再被内层清除。
- `run_real_probe.py` 固定顺序为 2-batch capture，然后分别用 batch00、batch01 的同一 bundle 做 old/block16 回放；每步要求预期成功 receipt，不在看到时间后重选 batch。原全局 resource_dispatch 仍逐个申请真实 lease。capture 的 VRAM/RSS 上限为 8192/32768MiB，replay 为 4096/8192MiB，manifest 明确它们是初始有界短测上限，不是伪造实测峰值；实际资源需 completion/monitor 回执才能接受。
- 固定学生 checkpoint 是已完成的 `rgbir_object_evidence_v1_20260906/full_weight0_s42_attempt1/weights/last.pt`；不是当前 C1 在跑模型的 live student。真实流依然是原 seed20260907 probe loader，而不是正式 sampler。两条 replay 均 warmup2/计时7次、交替顺序，计时只覆盖完整 selector+KD/statistics+raw-score 梯度，不含学生/教师前向与 optimizer。
- capture 的 selector seed=cfg.seed+batch_index+1，bundle replay 固定 20260907；原 paired `_choose` 不使用 seed，仅 paired_random 分支才使用，因此本任务 paired 选择不受该数值差异影响，不能将此结论推广到随机内容臂。

**修复版仅获上述代码范围复核；真实 capture、两次 replay、源副本身份、进程退出和资源完成结果仍待独立读取。长训切换没有准入。**
