...
`compact_bundle_cpu_receipt.json`另核对将S/T/R非零随机feature值替换为零stride占位后，原selector所有delta、valid、选择、质量、C0stats、loss及梯度exact，证明这一C1接口只需feature形状；不推广到feature KD。
...
combined反向的有限梯度不能证明KD有效，因此另以autograd.grad读取KD到S raw scores的梯度，要求有限且L2>0；这个检查不写参数.grad、保留主图供后续总反向，耗时单独记录并从pipeline墙钟跨度扣除。零/非有限时报告失败且不替换自然batch。
...
嵌套pool计时不重置CUDA峰值，只有外层重放重置并报告该完整区间峰值；内层只报同步墙钟，避免把最后一次pool峰值误当整段峰值。NVML/RSS完整进程树资源由既有lease guard记录。
