...
import compare_24_selected_only as m
...
class ComparisonTests(unittest.TestCase):
    ...
    def make_runs(self, root, change=None):
        for worker in ('old','selected_only'):
            ...
            m.write_json(path/'worker_receipt.json',dict(status='COMPLETED_24_UPDATE_DIAGNOSTIC',
                candidate_coverage=dict(thin_learning_batches=24 if worker=='selected_only' else 0,
                    full_diagnostics_batches=24,fallback_batches=0,extra_full_diagnostics_seconds=0.),
            ...
            for i in range(1,25):
                ...
                torch.save(dict(identity=dict(selected=torch.tensor([True])),floating=dict(delta=torch.zeros(1)),
                    delta_collection_scope='selected_S_T_only'),path/('selection_%04d.pt'%i))
                torch.save(dict(native_total=1.,kd_unweighted=.1,coefficient=m.COEFFICIENT),path/('loss_%04d.pt'%i))
                ...
                if change=='parameter' and worker=='selected_only' and i==24:state['after']['student']['p']+=2e-6
                torch.save(state,path/('attempt_%03d.pt'%i))
        if change=='intermediate':
            p=root/'selected_only'/'selection_0024.pt';r=m.load_pt(p);r['floating']['delta']+=2e-6;torch.save(r,p)
        if change=='identity':
            p=root/'selected_only'/'selection_0024.pt';r=m.load_pt(p);r['identity']['selected'][0]=False;torch.save(r,p)

    def run_comparison(self, change=None):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp);self.make_runs(root,change)
            return m.compare_runs(root/'old',root/'selected_only',root/'out')
    ...
    def test_missing_attempt_cannot_pass(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (root/'selected_only'/'attempt_024.pt').unlink()
            with self.assertRaises(AssertionError):m.compare_runs(root/'old',root/'selected_only',root/'out')

    def test_cuda_not_initialized(self):
        self.assertFalse(torch.cuda.is_initialized())

    def test_fallback_cannot_masquerade_as_thin_training(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp);self.make_runs(root)
            p=root/'selected_only'/'worker_receipt.json';receipt=json.loads(p.read_text())
            receipt['candidate_coverage'].update(thin_learning_batches=23,fallback_batches=1)
            m.write_json(p,receipt)
            with self.assertRaises(AssertionError):m.compare_runs(root/'old',root/'selected_only',root/'out')

    def test_missing_per_batch_loss_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp);self.make_runs(root)
            (root/'selected_only'/'loss_0004.pt').unlink()
            with self.assertRaises(AssertionError):m.compare_runs(root/'old',root/'selected_only',root/'out')

    def test_per_batch_kd_difference_fails_trajectory(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp);self.make_runs(root)
            p=root/'selected_only'/'loss_0004.pt';values=m.load_pt(p);values['kd_unweighted']+=1e-3;torch.save(values,p)
            result=m.compare_runs(root/'old',root/'selected_only',root/'out')
            self.assertFalse(result['trajectory_numeric_agreement'])
            self.assertFalse(result['categories']['per_batch_losses']['numeric_agreement'])
