"""Fresh-process old/block16 C1 24-update diagnostic; never production admission.
...
def selection_snapshot(s, helper):
    ...
    floating = {k: getattr(s, k) for k in ('student_delta', 'teacher_delta', 'reference_delta',
                                         'quality', 'c0_loss', 'c0_stats')}
...
def snapshot(args, out, cfg):
    sources = [Path(__file__).resolve(), args.pool_source.resolve()]
...
def run_worker(args):
    ...
    pool = explicit_module('_update24_candidate_pool',args.pool_source.resolve())
    ...
    holder = {}
    chosen_pool = selection_module.pool_relative_logits if args.worker == 'old' else pool.pool_relative_logits_block16
    selector = clone_function(selection_module.build_classification_selection, pool_relative_logits=chosen_pool)
    def observed_selector(*a, **kw):
        selected = selector(*a,**kw)
...
def compare_runs(old, new, output):
    ...
    for worker, receipt in zip(('old','block16'),receipts):
    ...
    files_old = {p.name for p in old.glob('*.pt') if p.name.startswith(('initial.','batch_','selection_','attempt_','final.','first_batch.'))}
    files_new = {p.name for p in new.glob('*.pt') if p.name.startswith(('initial.','batch_','selection_','attempt_','final.','first_batch.'))}
    ...
    trajectory = ('losses','gradients_scaled','student','optimizer','ema','applied_gradients')
    ...
    result = dict(status='COMPLETED_COMPARISON_NOT_PRODUCTION_ADMISSION',atol=ATOL,rtol=RTOL,
        ...
        mathematical_definition_claim='Same masked-LME expression is a source-level claim, not proved by a trajectory test.',
...
def main():
    ...
    parser.add_argument('--pool-source',type=Path,default=Path(__file__).resolve().parents[1]/'pool_block16.py')
    parser.add_argument('--output',type=Path,required=True)
    parser.add_argument('--worker',choices=('old','block16'),help=argparse.SUPPRESS)
    ...
    for worker in ('old','block16'):
        command=[sys.executable,str(Path(__file__).resolve()),'--reference-dir',str(args.reference_dir.resolve()),
            '--config',str(args.config.resolve()),'--pool-source',str(args.pool_source.resolve()),
            '--output',str(out/worker),'--worker',worker]
        subprocess.run(command,check=True)
    if torch.cuda.is_initialized():raise RuntimeError('Coordinator initialized CUDA')
    result=compare_runs(out/'old',out/'block16',out/'comparison')
