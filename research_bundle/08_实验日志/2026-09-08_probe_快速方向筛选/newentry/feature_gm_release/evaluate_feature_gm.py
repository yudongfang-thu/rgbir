"""Full-dev native FP32 evaluation of the independent feature-GM FT3 last/EMA."""
import argparse
import gzip
import json
import math
...
ARMS = ('N', 'F-rel-GM')
DATASET_ARMS = {'drone': ('N', 'F-rel-GM')}
POPULATIONS = {'drone': (1469, 22462, 5)}
...
def load_config(path):
    from feature_gm_common import load_config as fixed_feature_gm_config
...
def training_completion(cfg, checkpoint, config_path=None):
    ...
    if saved_config.resolve() != checkpoint.parent.parent/'feature_gm_config.yaml':
...
def run(args):
    ...
    shutil.copyfile(args.config, output/'feature_gm_config.yaml')
