...
import unittest
import yaml
import run_direction_queue as q
...
def calibration(dataset, checkpoint, blocked=False):
    ...
    return dict(status='DIRECTION_CALIBRATION_COMPLETED', dataset=dataset, coefficients=coefficients,
                blocked=reasons, scope='EXPLORATORY_FIXED8_BNFROZEN', batches=8, seed=42,
...
class Tests(unittest.TestCase):
    def fixture(self, dataset, blocked=False, fail=None, wrong_stream=False):
        ...
        def execute(job, queue):
            ...
            if stage == 'eval':
                value = dict(status='DIRECTION_EVALUATION_COMPLETED', scope=q.SCOPE, endpoint=q.ENDPOINT,
            ...
            value = dict(status='DIRECTION_CANARY_COMPLETED' if stage == 'canary' else 'DIRECTION_TRAINING_COMPLETED',
    ...
    def test_invalid_candidate_is_skipped_and_N_continues(self):
        out,cfgs,execute,calls=self.fixture('llvip',blocked=True)
        result=q.process_dataset(HERE,out,'llvip',cfgs,execute)
        self.assertEqual(result['status'],'COMPLETED_WITH_BLOCKED_ARMS',result)
        self.assertEqual(calls,[('calibration','N'),('canary','N'),('train','N'),('eval','N')])

    def test_technical_failure_stops_only_dataset(self):
        out,cfgs,execute,calls=self.fixture('llvip',fail=('canary','L2-box'))
        result=q.process_dataset(HERE,out,'llvip',cfgs,execute)
        self.assertEqual(result['status'],'DATASET_STOPPED_ON_TECHNICAL_FAILURE')
        self.assertNotIn(('train','N'),calls)
        out,cfgs,execute,calls=self.fixture('drone')
    ...
    def test_malformed_coefficients_fail_closed(self):
        ...
        for invalid in [float('nan'),0.,1.1,True,None]:
            bad=copy.deepcopy(receipt);bad['coefficients']['L2-box']=invalid
...
if __name__=='__main__':
    ...
    with (HERE/'DIRECTION_QUEUE_CPU_CHECKS.json').open('x',encoding='utf-8') as f:json.dump(receipt,f,indent=2)
