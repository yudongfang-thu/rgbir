# coding: utf-8
"""LLVIP then Drone direction screen on the existing global lease only.
...
DATASETS = ('llvip', 'drone')
ARMS = {'llvip': ('N', 'L2-box', 'L2-GT'), 'drone': ('N', 'C1', 'C2', 'F-rel')}
...
def coefficient_plan(dataset, receipt):
    if receipt.get('status') != 'DIRECTION_CALIBRATION_COMPLETED' or receipt.get('dataset') != dataset:
        raise ValueError('Calibration did not complete for this dataset')
    fixed = dict(scope='EXPLORATORY_FIXED8_BNFROZEN', batches=8, seed=42,
    ...
    if dataset == 'llvip':
        if ('L2-box' in accepted) != ('L2-GT' in accepted):
            raise ValueError('Shared-mask L2 arms must share calibration validity')
        if 'L2-box' in accepted and accepted['L2-box'] != accepted['L2-GT']:
            raise ValueError('Shared-mask L2-GT must retain the L2-box coefficient')
...
def effective_config(cfg, coefficient, calibration_path):
    ...
    value['localization_coefficient'] = coefficient if arm.startswith('L2') else 0.
...
def check_terminal(stage, dataset, arm, receipt):
    status = {'canary': 'DIRECTION_CANARY_COMPLETED', 'train': 'DIRECTION_TRAINING_COMPLETED',
              'eval': 'DIRECTION_EVALUATION_COMPLETED'}[stage]
...
def make_job(release, output, dataset, arm, stage, reference_dir=REFERENCE, budget=None, cfg=None):
    ...
    job = dict(id='direction_' + name + '_' + dataset + '_' + arm + '_' + stage,
    ...
    if stage == 'calibration':
        ...
        command = [str(PY), str(release / 'calibrate_direction.py'), '--reference-dir', str(reference_dir),
        ...
        command = [str(PY), str(release / 'train_direction.py'), '--reference-dir', str(reference_dir),
        ...
        command = [str(PY), str(release / 'evaluate_direction.py'), '--reference-dir', str(reference_dir),
...
def process_dataset(release, output, dataset, configs, executor, reference_dir=REFERENCE):
    ...
    try:
        ...
        write_new(queue / (dataset + '_canary_checks.json'), dict(status='DIRECTION_CANARIES_MATCHED',
...
def run(args):
    ...
    for path in (PY, reference / 'runtime.py', DISPATCH / 'resource_dispatch.py',
                 release / 'calibrate_direction.py', release / 'train_direction.py', release / 'evaluate_direction.py'):
        if not path.is_file():
    ...
    status = 'DIRECTION_MATRIX_PARTIAL_FAILURE' if failed else ('DIRECTION_MATRIX_COMPLETED_WITH_BLOCKED_ARMS' if blocked else 'DIRECTION_MATRIX_COMPLETED')
