"""Warm-start LLVIP N/C0 original object-evidence on a fixed 2048-image subset; fresh optimizer and EMA."""
import argparse
...
import types
import yaml
from confidence_common import ENDPOINT,copy_sources,data_output,explicit,load_config,read,stat,write_new
...
def build_private(runtime,cfg,args):
...
def run(args):
    ...
    if not args.canary:
        ...
        if c.get('status')!='CONFIDENCE_CANARY_COMPLETED' or c.get('arm')!=cfg['arm'] or c.get('successful_updates',0)<24 or c.get('scope')!='LLVIP_CONFIDENCE_FT3' or c.get('dataset')!='llvip' or c.get('bn_running_buffers_unchanged') is not True:
    ...
    import torch
    import runtime
    if Path(runtime.__file__).resolve()!=ref/'runtime.py':raise ValueError('Wrong pinned runtime')
    legacy=runtime.legacy
    ...
    build,extra=build_private(runtime,cfg,args)
    output.mkdir(parents=True,exist_ok=False);shutil.copyfile(args.config,output/'confidence_config.yaml')
    ...
    try:
        ...
        result=dict(status='CONFIDENCE_CANARY_COMPLETED' if args.canary else 'CONFIDENCE_TRAINING_COMPLETED',
            scope='LLVIP_CONFIDENCE_FT3',single_seed=True,arm=cfg['arm'],seed=42,dataset=cfg['dataset'],
            epochs_configured=3,last_epoch=trainer.epoch+1,endpoint=ENDPOINT,formal_e200_complete=False,
            config_copy=str(output/'confidence_config.yaml'),model=cfg['model'],teacher=cfg['teacher'],reference=cfg['reference'],
        ...
        write_new(output/'confidence_failure.json',dict(status='CONFIDENCE_TRAINING_FAILED',scope='LLVIP_CONFIDENCE_FT3',error=repr(error),
