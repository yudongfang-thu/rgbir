...
def load_config(path):
    from confidence_common import load_config as fixed_confidence_config
    ...
    if cfg.get('confidence_screen', {}).get('scope', SCOPE) != SCOPE:
...
def training_completion(cfg, checkpoint, config_path=None):
    ...
    fixed = dict(status='CONFIDENCE_TRAINING_COMPLETED', scope=SCOPE, endpoint=ENDPOINT,
    ...
    if saved_config.resolve() != checkpoint.parent.parent/'confidence_config.yaml':
...
def run(args):
    ...
    shutil.copyfile(args.config, output/'confidence_config.yaml')
