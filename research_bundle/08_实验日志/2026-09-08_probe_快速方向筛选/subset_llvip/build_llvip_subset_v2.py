...
def main():
    ...
    for v, t in sorted(original_mapping.items()):
        vp, tp = Path(v), Path(t)
        assert vp.stem == tp.stem and v == fit[vp.stem]['visible'] and t == fit[vp.stem]['infrared']
        alias_v = v_root / 'images/fit' / vp.name
        alias_t = t_root / 'images/fit' / tp.name
        assert alias_v.resolve(strict=True) == vp.resolve(strict=True) and alias_t.resolve(strict=True) == tp.resolve(strict=True)
        ...
        rows.append({'rgb': str(alias_v), 'visible': str(alias_v), 'infrared': str(alias_t), 'mapping_visible': v, 'mapping_infrared': t, 'source_group': fit[vp.stem]['sequence_prefix'],
    ...
    write_json('visible_to_infrared_train.json', {x['mapping_visible']: x['mapping_infrared'] for x in selected})
    ...
    manifest = {'status': 'CPU_SUBSET_CREATED_NO_TRAINING', 'dataset': 'llvip',
                ...
                'checks': {'full_fit_mapping_and_governed_stems_exact': True, 'processed_to_governed_raw_targets_exact': True,
                           ...
                           'processed_aliases_preserved_for_label_loading': True, 'mapping_canonical_original_subset_exact': True},
