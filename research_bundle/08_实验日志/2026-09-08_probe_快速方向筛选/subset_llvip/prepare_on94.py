...
def main():
    receipt_path = HERE / 'remote_build_receipt.json'
    if receipt_path.exists():
        raise FileExistsError(receipt_path)
    files = {n: base64.b64encode((HERE / n).read_bytes()).decode() for n in ['build_llvip_subset.py', 'natural_sampler_source.py']}
    code = '''import base64,json,subprocess,sys
from pathlib import Path
spec=SPEC
p=Path(PARENT)/'subset_builder_llvip_v1'
...
subprocess.run([sys.executable,str(p/'build_llvip_subset.py'),'--self-test'],check=True)
subprocess.run([sys.executable,str(p/'build_llvip_subset.py'),'--output',OUTPUT],check=True)
