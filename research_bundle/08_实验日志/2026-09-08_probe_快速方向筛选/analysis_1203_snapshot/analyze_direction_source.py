...
ARMS = {'drone': ('N','C1','C2','F-rel'), 'llvip': ('N','L2-box','L2-GT')}
POPULATIONS = {'drone': (1469,22462,5), 'llvip': (2406,7879,1)}
AP = ('mAP50_95','AP50','AP75')
METRICS = AP + ('precision','recall')
CONTRASTS = {'drone': (('C1','N'),('C2','N'),('F-rel','N'),('C2','C1'),('F-rel','C1')),
             'llvip': (('L2-box','N'),('L2-GT','N'),('L2-box','L2-GT'))}
...
def summarize(receipts, unavailable=None):
    ...
    for ds,arms in ARMS.items():
        ...
        if ds=='llvip' and 'L2-box' in valid and 'L2-GT' in valid:
            require(valid['L2-box']['kd_coefficient']==valid['L2-GT']['kd_coefficient'],
                    'LLVIP L2-box/L2-GT must share the fixed teacher-mask coefficient')
    ...
    return dict(status='COMPLETE_DIRECTION_READOUT' if complete else 'PARTIAL_DIRECTION_READOUT',scope=SCOPE,
    ...
    for ds,arms in ARMS.items():
        for arm in arms:
            ...
            if p.is_file() and f.is_file():
                ...
                fail=read(f)
                require(fail.get('status')=='DIRECTION_EVALUATION_FAILED' and fail.get('scope')==SCOPE, 'Unexpected failure receipt scope')
...
def main():
    ...
    shutil.copyfile(__file__,args.output/'analyze_direction_source.py')
    require(Path(__file__).read_bytes()==(args.output/'analyze_direction_source.py').read_bytes(),'Analyzer source copy differs')
    print(result['status'])
