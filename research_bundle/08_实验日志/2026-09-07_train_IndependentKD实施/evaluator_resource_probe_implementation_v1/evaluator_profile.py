...
def run(args):
    ...
    sources.update(Path(p) for p in legacy.implementation_files(DetectionValidator,BaseValidator,YOLO.val,
    ...
    try:
        for label,validator_class in (('native',DetectionValidator),('evidence',make_evidence_validator(DetectionValidator))):
            ...
            def on_start(v):
                ...
                for obj in (type(v.dataloader),type(v.dataloader.dataset),type(v.model)):
                    path=inspect.getsourcefile(obj)
                    if path:sources.add(Path(path))
            def on_end(v):
                if v.seen!=1469:
