...
**状态：IMPLEMENTED / NOT_ACCEPTED；只经合成已知真值 CPU 验证，未分析新 C1 AP，未独立接受。** 2026-09-07，规则在读取本轮新方法对象结果前冻结。实现位于本实验日志，未改训练 release 或已接受分析器。
...
尺度使用实际输入网络的 xyxy 框面积，要求 max(canvas_shape)=640（允许640×较短边的 rect canvas），不使用原图面积、不把它称 COCO 原图尺度：`canvas640_small_lt32sq`（area<32²）、`canvas640_medium_32sq_to_lt96sq`（32²≤area<96²）、`canvas640_large_ge96sq`（area≥96²）。GT分组用GT框，背景FP尺度用预测框；两种总体分别报告。
