...
目录必须存在三个真实 C1 paired seed42/0/123、E200、FROZEN、formal_training_authorized=true 的配置；逐个调用该 release 的 protocol / admission 检查。profile 必须是实际完成的 canary / evaluation_profile，保留其原始文件及测量，调用真实 `measured_reservation` 验证源码/config/data绑定；不从假峰值、calibration或布尔ready推导。正式预约固定为VRAM ceil(实际峰值)+256MiB，RSS max(8192MiB, ceil(实际进程树峰值)+4096MiB)，写入manifest，不把bootstrap16000MiB上限当实测需要。缺少任一项就不能 prepare。单卡独测profile是否允许与项目其他训练共享，仍由真实dispatcher的companion规则判断。
...
作者侧15/15测试通过（0.228s），其中包括将1370MiB显存/6260MiB RSS的合成测量固定预约为1626/10356MiB，而非bootstrap16000/49152。独立代码审阅另行进行，不以本测试声明接受。
