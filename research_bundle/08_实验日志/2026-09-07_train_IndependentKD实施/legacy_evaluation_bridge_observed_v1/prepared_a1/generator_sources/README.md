...
主要产物为 `evaluation_compatibility_candidate.json`、保留旧 protocol_id 与原臂/seed 的 `manifest_candidate.json`、`summary.json`、每项旧来源独立副本、新实际评估合同/指标/objects/所有九源，以及明确的本地/服务器来源映射。对象只复制一份；原 receipt 中另一份已验证相同的对象快照仍保留在原完成快照内。
