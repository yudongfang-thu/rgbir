...
def prepare(args):
    ...
    for spec, old, folder, remote, result, resource_path in validated:
        ...
        snapshot_copies = {}
        for relative in result['receipt']['metric_snapshots']:
            source = folder/'eval_evidence'/relative
            same = [name for name in ('evaluation_val.json', 'predictions/objects.jsonl.gz',
                    'historical_metric_comparison.json') if (folder/name).read_bytes() == source.read_bytes()]
            require(len(same) == 1, 'Actual metric snapshot has no unique retained independent payload')
            retained = root+'/observed/'+same[0]
            snapshot_copies[relative] = retained
            mappings.append(dict(local_source=str(source.resolve()), source_remote=remote+'/eval_evidence/'+relative,
                independent_copy=retained, bytes=source.stat().st_size,
                binding_kind='byte-identical receipt snapshot alias; payload copied once'))
        ...
        entry = dict(checkpoint=result['metric']['checkpoint'], seed=spec['seed'], actual_source_arm=METHODS[spec['arm']],
            ...
            observed_reevaluation=dict(remote_attempt=remote, local_snapshot=str(folder.resolve()),
                ...
                metric_snapshot_independent_copies=snapshot_copies,
