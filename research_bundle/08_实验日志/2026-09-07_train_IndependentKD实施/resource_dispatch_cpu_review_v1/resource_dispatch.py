...
First measurements are short bootstrap stages on GPUs with no process or lease.
"""
...
DEFAULT_REPO = Path('/mnt/dataset/yudongfang/projects/SpaceNet6_OTD_official_reproduction')
...
BOOTSTRAP_STAGES = ('calibration', 'canary', 'compatibility')
...
def validate_job(job):
    ...
    if job['stage'] not in ('calibration', 'canary', 'compatibility', 'train', 'evaluation', 'feature'):
    ...
    if job['stage'] in ('calibration', 'canary', 'compatibility', 'train') and not job.get('config'):
...
def measured_reservation(job):
    ...
    elif profile.get('stage') in ('evaluation', 'feature'):
    ...
    return dict(bootstrap=False, measured=True, vram_mib=job['vram_mib'],
                rss_mib=job['rss_mib'], profiled_second_train=True,
...
def admission_candidates(usage, rows, job, reservation, guard_max_gpus=3):
    ...
    for gpu in sorted(rows):
        ...
        if reservation['bootstrap'] and gpu not in empty:
            why.append('first measurement requires a GPU with no process or lease')
    ...
    with (output / (job['id'] + '.log')).open('x', encoding='utf-8') as log:
        ...
        try:
            ...
            report = dict(schema=PROFILE_SCHEMA, status='COMPLETED', measurement_valid=True,
                          stage=job['stage'], profile_key=job['profile_key'], profile_identity=job.get('profile_identity'),
...
def ordered_jobs(jobs):
    ...
    return sorted(jobs, key=lambda j: (0 if j['stage'] == 'evaluation' else 1,
                                      j.get('priority', 0)))
