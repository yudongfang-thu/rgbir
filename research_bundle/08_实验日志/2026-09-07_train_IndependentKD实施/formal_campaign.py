...
ORDER = (42,0,123)
POLL_SECONDS = 30
TRAIN_VRAM_MARGIN_MIB = 2048
EVALUATION_VRAM_MARGIN_MIB = 256
RSS_MARGIN_MIB = 4096
...
def profile_resources(profile_path,expected_stage):
    ...
    margin = TRAIN_VRAM_MARGIN_MIB if expected_stage=='canary' else EVALUATION_VRAM_MARGIN_MIB
    return profile,dict(vram_mib=math.ceil(peaks['vram_mib'])+margin,
...
def initialize(args):
    ...
    for seed in ORDER:
        ...
        row=build_jobs(release,campaign,Path(args.python).absolute(),seed,path,
    ...
    manifest=dict(schema=SCHEMA,release=str(release),repo=str(Path(args.repo).resolve()),
        python=str(Path(args.python).absolute()),worker_script=str(Path(__file__).resolve()),seed_order=list(ORDER),
        seeds=seeds,input_snapshots=snapshots,status='PREPARED_NOT_LAUNCHED',no_ap_early_stop=True,
        measured_reservation_policy=dict(train_vram_margin_mib=TRAIN_VRAM_MARGIN_MIB,
            evaluation_vram_margin_mib=EVALUATION_VRAM_MARGIN_MIB,rss_margin_mib=RSS_MARGIN_MIB,
            rss_minimum_mib=RSS_MINIMUM_MIB,rounding='ceil each actual measured peak in MiB',
            training_margin_reason='Current C1 canary measured peak plus 2048 MiB for native TAL batch GT-density variation; no historical C0 peak substitution'),
