...
class AnalyzerTests(unittest.TestCase):
    ...
    def test_legacy_evaluator_bridge_binds_configs_and_source_copies(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp) / 'run'; spec = file_fixture(root)
            path = root / 'eval_evidence/run_receipt.json'
            data = json.loads(path.read_text()); data['source_snapshots']['config'] = ['config.yaml']
            path.write_text(json.dumps(data))
            source_copy = Path(tmp) / 'accepted_old_evaluator.py'
            source_copy.write_bytes((root / 'eval_evidence/evaluator.py').read_bytes())
            canonical = Path(tmp) / 'accepted_canonical_evaluator.py'
            canonical.write_bytes(source_copy.read_bytes())
            metric = json.loads((root / 'evaluation_val.json').read_text())
            entry = {'checkpoint': metric['checkpoint'], 'seed': 42,
                     'bound_training_config': json.loads((root / 'run_evidence/config.yaml').read_text()),
                     'bound_evaluation_config': json.loads((root / 'eval_evidence/config.yaml').read_text()),
                     'evaluation_source_copies': {'evaluator.py': str(source_copy)},
                     'evaluation_contract': json.loads((root / 'eval_evidence/evaluation_contract.json').read_text())}
            bridge = Path(tmp) / 'bridge.json'
            bridge.write_text(json.dumps({'status': 'ACCEPTED', 'reviewer': 'synthetic independent reviewer',
                                         'entries': [entry], 'canonical_evaluator_source_copies': [str(canonical)]}))
            spec['evaluation_compatibility_receipt'] = str(bridge)
            self.assertTrue(validate_endpoint(load_endpoint(spec, tmp))['valid'])
            source_copy.write_text('# changed after acceptance\n')
