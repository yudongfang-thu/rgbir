...
SCHEMA = 'rgbir-object-error-analysis-v2'
UNKNOWN = 'UNKNOWN'
SCALE_NAMES = ('input640_small_lt32sq', 'input640_medium_32sq_to_lt96sq', 'input640_large_ge96sq')
...
def validate_row(row, nc=None):
    ...
    for field in ('canvas_shape','original_shape'):
        shape = row.get(field)
        if (not isinstance(shape, list) or len(shape) != 2 or
                any(not finite(v) or int(v) != v or v <= 0 for v in shape)):
            raise ValueError('Require finite positive integer dimensions: '+field)
    for kind in ('gt', 'pred'):
...
def observed_batch_geometry(indexed, contract):
    """Audit bound object shapes in the contract's actual loader order.

    The contract records order/settings, not shapes. Shapes below come only
    from the separately receipt-bound objects; no pixels or boxes are changed.
    """
    effective=contract.get('effective_kwargs',{})
    if effective.get('imgsz')!=640:
        raise ValueError('Require receipt-bound nominal input imgsz=640')
    batch=effective.get('batch');order=contract.get('actual_loader_roster')
    if type(batch) is not int or batch<=0 or not isinstance(order,list) or len(order)!=len(indexed) or set(order)!=set(indexed):
        raise ValueError('Missing actual loader order or batch setting for observed canvas audit')
    batches=[]
    for start in range(0,len(order),batch):
        rows=[indexed[image] for image in order[start:start+batch]]
        shapes={tuple(row['canvas_shape']) for row in rows}
        if len(shapes)!=1:raise ValueError('Receipt-bound object canvas differs within one observed batch')
        batches.append(dict(batch_index=start//batch,images=len(rows),canvas_shape=list(next(iter(shapes))),
            original_shapes=[list(shape) for shape in sorted({tuple(row['original_shape']) for row in rows})]))
    return dict(nominal_imgsz=640,batch_size=batch,batches=batches,
        canvas_shape_source='receipt-bound objects.jsonl.gz; not an extra contract shape measurement',
        order_source='receipt-bound evaluation_contract.actual_loader_roster',boxes_rescaled=False)
...
def load_evaluation(metric_path):
    ...
    geometry=observed_batch_geometry(indexed,contract)
    return dict(raw=raw,contract=contract,config=cfg,rows=rows,nc=nc,
        observed_batch_geometry=geometry,
...
def run(args):
    ...
    result.update(analyzer_status='ACCEPTED' if accepted else 'NOT_ACCEPTED',**{contract_field:ERROR_CONTRACT},
        observed_batch_geometry={'baseline':a['observed_batch_geometry'],'candidate':b['observed_batch_geometry']},
