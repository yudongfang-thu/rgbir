...
def verify_one(cfg, config_path, output):
    ...
    loader_dir.mkdir()
    started=time.time()
    loaders=verify_loaders(cfg,loader_dir)
    write_json(output/"loader_receipt.json",loaders)
    historical_output=output/"historical"
    old=run_trajectory(cfg,config_path,historical_output,historical_output,True)
    new=run_trajectory(cfg,config_path,output/"new",historical_output,False)
...
def run(args):
    ...
    if resolved==Path(__file__).resolve().parent or Path(__file__).resolve().parent in resolved.parents:
