"""Mirror and publish completed single-batch DFL evidence only; never alter original outputs."""
...
for p in sorted(root.rglob('*')):
    ...
    files.append((rel,b))
if mode=='mirror':
    dest='/mnt/dataset/yudongfang/projects/RGBT_campaign/artifacts/rgbir_dfl_information_20260908/review_v1'
    archive=io.BytesIO()
...
r=dict(status='RAW_DFL_INFORMATION_EVIDENCE_MIRRORED',destination=str(root),files=rows,new_hash_computed=False)
...
"""%dest
    ...
    lines=old.decode('utf-8').splitlines();marker='[probe_DFL真实信息读出]('
    ...
    banner='> **2026-09-08 单批真实DFL信息读出完成**：固定32图80GT，保存真实四边DFL分布、同次解码及逐对象GT读出，区分额外信息和可迁移性。见[完整结果与边界](research_bundle/08_实验日志/2026-09-08_probe_DFL真实信息读出/FINAL_REPORT.md)。'
    if not any(x.startswith('> **2026-09-08 单批真实DFL信息读出完成') for x in lines):lines=[banner,'']+lines
    p.write_bytes(('\n'.join(lines)+'\n').encode('utf-8'))
    checks=repo/'publication_checks/update_20260908_raw_dfl_information';checks.mkdir(parents=True,exist_ok=True)
    ...
    print('RAW_DFL_INFORMATION_PUBLISHED',len(files),sum(x['bytes'] for x in rows))
