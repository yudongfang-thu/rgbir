...
import types
import yaml
from direction_common import ENDPOINT,copy_sources,data_output,explicit,load_config,read,stat,write_new
...
def run(args):
    ...
    if not args.canary:
        ...
        if c.get('status')!='DIRECTION_CANARY_COMPLETED' or c.get('arm')!=cfg['arm'] or c.get('successful_updates',0)<24:
    ...
    try:
        ...
        result=dict(status='DIRECTION_CANARY_COMPLETED' if args.canary else 'DIRECTION_TRAINING_COMPLETED',
            scope='DIRECTION_FT3_BNFROZEN',single_seed=True,arm=cfg['arm'],seed=42,dataset=cfg['dataset'],
        ...
        write_new(output/'direction_failure.json',dict(status='HOURLY_SCREEN_FAILED',error=repr(error),
