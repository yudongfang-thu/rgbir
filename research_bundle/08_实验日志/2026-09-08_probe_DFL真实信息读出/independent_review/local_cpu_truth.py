...
def main(a):
    ...
    frames=load_rows(witness.with_name('witness_frames.jsonl'))
    checked('historical_full_population',[len(rows),len(by_id),len(set(r['stable_rgb_gt_id'] for r in rows)),len(frames),len(set(r['image_index'] for r in rows))],[80,80,80,32,31])
    for r in rows:
