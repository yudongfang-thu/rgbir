...
if __name__=='__main__':
    try:
        ...
        # Preserve the current arm alongside a terminal status for later inspection.
        if '--root' in sys.argv and '--stage' in sys.argv:
            root=Path(sys.argv[sys.argv.index('--root')+1])
            stage=sys.argv[sys.argv.index('--stage')+1]
            status_path=root/f'{stage}_queue_status.json'
            status=json.loads(status_path.read_text()) if status_path.exists() else {}
            status.update(status='failed',error=repr(error),updated_at=datetime.datetime.now(datetime.timezone.utc).isoformat())
