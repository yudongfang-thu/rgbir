...
class Truths(unittest.TestCase):
    ...
    def test_native_independent_iou_changes_prediction_identity(self):
        ...
        for threshold in (.5,.75):
            ...
            pairs,tp=call_with_pairs(v,B,pred,gt);results.append((set(pairs[:,0]),set(pairs[:,1])))
        self.assertEqual(results,[({1,2},{0,1}),({1,2},{1,2})]);self.assertIs(sys.getprofile(),prior)
