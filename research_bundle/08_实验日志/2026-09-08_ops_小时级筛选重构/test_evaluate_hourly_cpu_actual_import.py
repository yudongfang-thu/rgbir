...
import hourly_common as helpers
...
result = dict(status='PASS_CPU_EVALUATION_ADAPTER_FIXTURES' if all(checks.values()) else 'FAIL',
              checks=checks, count=len(checks), common_helpers='Actual release/hourly_common.py imported; no stub',
              real_receipts_used=False, real_checkpoint_loaded=False, gpu=False, new_hashes=False)
with (HERE / 'evaluation_cpu_checks_actual_import.json').open('x', encoding='utf-8') as f:
