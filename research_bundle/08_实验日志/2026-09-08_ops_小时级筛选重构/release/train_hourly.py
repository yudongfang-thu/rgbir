"""Warm-start N/C0/C1 on a fixed 2048-image subset; fresh optimizer and EMA."""
import argparse
...
import types
import yaml
from hourly_common import ENDPOINT,copy_sources,data_output,explicit,load_config,read,stat,write_new
...
def build_private(runtime,criterion,selection,classification,cfg,args):
    legacy=runtime.legacy;parent=criterion.IndependentCriterion;extra=[]
    if cfg['arm']=='C1':
        p=Path(cfg['hourly_screen']['candidate_source'])
        if p.read_bytes()!=Path(cfg['hourly_screen']['approved_candidate_source']).read_bytes():
            raise ValueError('Selected-only implementation differs from executed approved source')
        m=explicit('_hourly_selected_only',p)
        parent=m.make_criterion_type(criterion,m.make_api(selection,classification));extra=[p]
    # First-batch gradient checks are sufficient here: operator equivalence was
    # already executed for this candidate. No per-batch heavy sanity logging.
    class ScreenCriterion(parent):
        def __call__(self,prediction,batch):
            self.sanity=bool(args.canary and self.calls==0)
    ...
    def frozen(path,expected_data,names):
        ...
        if key is None or str(expected_data)!=cfg['paths'][key]:raise ValueError('Unexpected frozen model binding')
...
def run(args):
    ...
    if not args.canary:
        ...
        if c.get('status')!='HOURLY_SCREEN_CANARY_COMPLETED' or c.get('arm')!=cfg['arm'] or c.get('successful_updates',0)<24:
    ...
    output.mkdir(parents=True,exist_ok=False);shutil.copyfile(args.config,output/'hourly_config.yaml')
    ...
    try:
        ...
        def setup():
            ...
            if (ready['train_images'],ready['val_images'],ready['train_batches'],ready['batch'],ready['workers'])!=(2048,1469,64,32,4):
        ...
        if args.canary:
            ...
            if not active or trainer.criterion_ref.selected_total<=0:raise RuntimeError('No nonzero selected KD signal')
        ...
        result=dict(status='HOURLY_SCREEN_CANARY_COMPLETED' if args.canary else 'HOURLY_SCREEN_TRAINING_COMPLETED',
            scope='HOURLY_SCREEN_FT',single_seed=True,arm=cfg['arm'],seed=42,dataset='dronevehicle',
            epochs_configured=3,last_epoch=trainer.epoch+1,endpoint=ENDPOINT,formal_e200_complete=False,
            config_copy=str(output/'hourly_config.yaml'),model=cfg['model'],teacher=cfg['teacher'],reference=cfg['reference'],
        ...
        write_new(output/('canary.json' if args.canary else 'hourly_training_receipt.json'),result)
        print(result['status'],cfg['arm'],result['seconds'],flush=True)
    except BaseException as error:
        write_new(output/'hourly_failure.json',dict(status='HOURLY_SCREEN_FAILED',error=repr(error),
