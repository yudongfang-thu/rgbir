...
predict uses fixed 640 letterbox. Per-image centered CKA uses native-scale tokens,
...
def matched_labels(a,b):
    ...
    eligible=score>=.1
    ia,ib=linear_sum_assignment(-(eligible*(min(score.shape)+1+score)))
    out=[]
...
def masks(shape,gt,grid=POOL):
    h,w=shape[:2];ratio=min(SIZE/h,SIZE/w)
    left=round((SIZE-round(w*ratio))/2-.1);top=round((SIZE-round(h*ratio))/2-.1)
    yy,xx=np.meshgrid((np.arange(grid)+.5)*SIZE/grid,(np.arange(grid)+.5)*SIZE/grid,indexing='ij')
...
def gt_errors(gt,pred):
    ...
    best=np.zeros(len(gt));assigned=np.zeros(len(gt));hit=np.zeros(len(gt),bool);wrong=np.zeros(len(gt),bool)
    if score.size:
        best=score.max(1)
        eligible=score>=.5
        ii,jj=linear_sum_assignment(-(eligible*(min(score.shape)+1+score)))
        for i,j in zip(ii,jj):
            if eligible[i,j]:hit[i]=True;assigned[i]=score[i,j]
        wrong=((mat>=.5)&(gt[:,None,0]!=p[None,:,5])).any(1)
    return [{'iou':float(assigned[i] if hit[i] else v),'hit':bool(hit[i]),
             'best_candidate_iou':float(v),'wrong_class_overlap':bool(wrong[i])} for i,v in enumerate(best)]
...
def predict_model(ckpt,paths,out,tag):
    ...
    with torch.inference_mode():
        for i,p in enumerate(paths):
            ...
            feats.append([z[0].cpu().half() for z in f])
            energies.append([torch.linalg.vector_norm(z,dim=1)[0].cpu().numpy() for z in f])
            boxes=r.boxes
...
def main():
    ...
    for id in ids:
        ...
        items.append({'id':id,'paths':paths,'gt':gt,'shapes':[im.shape[:2] for im in ims],
                      'valid':va&vb,'fg':(fa|fb)&va&vb,'match':matched_labels(*gt)})
    ...
    dump(out/'sample_files.json',[{'id':it['id'],'files':[{'path':str(p),'sha256':digest(p)} for p in it['paths']],
         'labels':[{'path':str(r/(it['id']+'.txt')),'sha256':digest(r/(it['id']+'.txt'))} for r in labroots]} for it in items])
    feat=[];energy=[];pred=[];peaks=[];model_names=[]
    for side,ck in enumerate(cfg['checkpoints']):
    ...
    if model_names[0]!=model_names[1]:raise ValueError('model class name order mismatch')
    dump(out/'class_names.json',model_names)
    dump(out/'prediction_records.json',clean([{'id':it['id'],'paths':[str(p) for p in it['paths']],
         'gt_rgb':it['gt'][0],'gt_ir':it['gt'][1],'pred_rgb':pred[0][i],'pred_ir':pred[1][i]} for i,it in enumerate(items)]))
    for it in items:
        it['masks_by_level']={}
        for li,lv in enumerate(LEVELS):
            grid=int(feat[0][0][li].shape[-1])
            va,fa=masks(it['shapes'][0],it['gt'][0],grid);vb,fb=masks(it['shapes'][1],it['gt'][1],grid)
            it['masks_by_level'][lv]={'all':va&vb,'fg':(fa|fb)&va&vb,'bg':va&vb&~(fa|fb)}
    permutations=[]
    if len(items)>1:
        for _ in range(5):
    ...
    for i,it in enumerate(items):
        ...
        for li,lv in enumerate(LEVELS):
            for region,mask in it['masks_by_level'][lv].items():
                pc=cka(feat[0][i][li],feat[1][i][li],mask)
                nc=[cka(feat[0][i][li],feat[1][perm[i]][li],mask & items[perm[i]]['masks_by_level'][lv]['all']) for perm in permutations]
                nc=[v for v in nc if v is not None];nm=float(np.mean(nc)) if nc else None
                ec=corr(energy[0][i][li],energy[1][i][li],mask)
                ne=[corr(energy[0][i][li],energy[1][perm[i]][li],mask & items[perm[i]]['masks_by_level'][lv]['all']) for perm in permutations]
    ...
    stats={'dataset':a.dataset,'n_images':len(items),'sample_seed':SEED,'single_checkpoint_seed':42,
        ...
        'limitations':['Annotation matching threshold IoU>=0.1 is a proxy; unpaired labels do not establish modality invisibility.',
          ...
          'Foreground CKA uses native-scale tokens and union GT; <8 tokens is N/A. Dimensions differ by layer; do not infer transferability ranking from CKA alone.',
