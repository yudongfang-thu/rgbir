...
def summarize(receipts,unavailable=None):
    ...
    for arm in ARMS:
        if arm not in valid:
            ...
            require(state.get('status') in ('MISSING','FAILED','INCOMPLETE','CONFLICT'),'Invalid unavailable status')
...
def collect(campaign):
    ...
    for arm in ARMS:
        ...
        if p.is_file() and f.is_file():unavailable[arm]=dict(status='CONFLICT',reason='Completed and failed receipts coexist; no outcome selected')
            ...
            failure=read(f);require(failure.get('status') in ('SUBSET_SCREEN_EVALUATION_FAILED','SUBSET_SCREEN_INCOMPLETE'),'Unexpected failure status')
            require(failure.get('scope')==SCOPE,'Foreign or missing failure scope')
            unavailable[arm]=dict(status='INCOMPLETE' if failure['status']=='SUBSET_SCREEN_INCOMPLETE' else 'FAILED',reason=str(failure.get('error','Evaluation failed')))
...
def markdown(result):
    ...
    out+=['','Primary observed direction: '+str(result['primary_direction'])+'. Missing/failed/incomplete/conflicting inputs are null, never zero. Per-class raw values, P/R and fixed C0-N differences are in summary.json.',
