...
REMOTE='/mnt/dataset/yudongfang/projects/RGBT_campaign/artifacts/rgbir_subset_e8_check_20260908/review_v1'
PY='/mnt/dataset/yudongfang/projects/SpaceNet6_OTD_official_reproduction/environments/sn6-int8-kd/bin/python'
MARKER='[probe_子集筛选器校验]('
CHECKS='publication_checks/update_20260908_subset_e8'
BANNER_PREFIX='> **2026-09-08 子集E8筛选器校验'
...
def collect(root=ROOT):
    ...
    for directory,dirs,names in os.walk(root,topdown=True,followlinks=False):
        ...
        for name in sorted(dirs):
            ...
            if (child.is_symlink() or resolved_root not in child.resolve().parents
                    or name.lower() in ('__pycache__','.git','weights','flow_reference','credentials','node_modules','.venv')):
    ...
    for path in sorted(candidates):
        ...
        elif any(p in ('__pycache__','.git','weights','flow_reference','credentials','node_modules','.venv') for p in lower):reason='excluded_directory'
        ...
        scan=data
        if compressed_json:
            # Native captures contain concatenated gzip members. Inspect every
            # decoded byte without materializing all predictions in memory.
            decoded_bytes=0;tail=b''
            with gzip.GzipFile(fileobj=io.BytesIO(data),mode='rb') as stream:
                while True:
                    chunk=stream.read(1_048_576)
                    if not chunk:break
                    decoded_bytes+=len(chunk)
                    if decoded_bytes>1_073_741_824:raise ValueError('Decompressed evidence exceeds 1GiB scan limit: '+str(rel))
                    window=tail+chunk
                    if CREDENTIAL_MARKER.search(window):raise ValueError('Credential marker: '+str(rel))
                    tail=window[-256:]
            scan=b''
...
def plan(files,excluded,state,note):
    return dict(scope='SUBSET_E8_CHECK_EVIDENCE_PUBLICATION',execution_state=state,state_note=note,
        entry=ROOT.name,files=public_rows(files),excluded=excluded,total_bytes=sum(row['bytes'] for row in files),
        new_hash_computed=False,weights_uploaded=False,images_uploaded=False,server_only_flow_tensors_excluded=True,
...
receipt=dict(status='SUBSET_E8_CHECK_EVIDENCE_MIRRORED',destination=str(root),files=rows,new_hash_computed=False)
...
def publish(files,excluded,args):
    ...
    def update_banner(lines):
        ...
        if len(matches)>1:raise ValueError('Duplicate subset-E8-check banners')
    ...
    print('SUBSET_E8_CHECK_EVIDENCE_WORKTREE_PREPARED',len(copied),args.state)
...
def main():
    ...
    if args.mode=='manifest':
        ...
        print('SUBSET_E8_CHECK_EVIDENCE_MANIFEST_PREPARED',len(files),args.state)
