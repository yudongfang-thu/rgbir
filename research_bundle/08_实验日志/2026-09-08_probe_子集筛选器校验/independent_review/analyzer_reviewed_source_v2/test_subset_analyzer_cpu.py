...
class Truths(unittest.TestCase):
    ...
    def test_missing_and_failed_not_zero(self):
        for unavailable in ({},{'C0':dict(status='FAILED',reason='synthetic')},{'C0':dict(status='INCOMPLETE',reason='synthetic deadline')}):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            path=folder/'short_evaluation_failure.json'
            for status,expected in [('SUBSET_SCREEN_EVALUATION_FAILED','FAILED'),('SUBSET_SCREEN_INCOMPLETE','INCOMPLETE')]:
                value=dict(status=status,scope=a.SCOPE,error='synthetic deadline')
                path.write_text(json.dumps(value))
                r=a.collect(Path(tmp));self.assertEqual(r['arms'][0]['status'],expected)
                self.assertIsNone(r['arms'][0]['raw_fraction']);self.assertIsNone(r['comparison']['delta_pp'])
                value['scope']='other';path.write_text(json.dumps(value))
                with self.assertRaises(ValueError):a.collect(Path(tmp))
