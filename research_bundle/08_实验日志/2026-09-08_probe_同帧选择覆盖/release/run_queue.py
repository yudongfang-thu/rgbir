...
def run(a):
    ...
    for p in (Path(PY),cfg,expected,DISPATCH/'resource_dispatch.py',release/'run_probe.py'):
        if not p.is_file():raise FileNotFoundError(p)
    shutil.copyfile(__file__,q/'executed_driver.py')
    job=dict(id='selection_coverage_'+output.name,kind='feature',formal=False,vram_mib=8192,rss_mib=32768,
        expected_receipt=str(output/'probe/completion_receipt.json'),
        command=[PY,str(release/'run_probe.py'),'--screen-release',str(SCREEN),'--reference-dir',str(REFERENCE),
            '--config',str(cp),'--expected-stream',str(expected),'--output',str(output/'probe')])
    ...
    try:
        ...
        for k,v in dict(status='SAME_FORWARD_SELECTION_COVERAGE_COMPLETED',batches=1,frames=32,
            optimizer_updates=0,backward=0,training=0,first_batch_stream_exact=True).items():
            if receipt.get(k)!=v:raise ValueError('Producer contract differs: '+k)
        write_new(q/'completion.json',dict(status='SAME_FORWARD_SELECTION_QUEUE_COMPLETED',receipt=job['expected_receipt'],
            seconds=time.time()-started,resource_status=result['status'],new_resource_pool=False,new_hash_computed=False))
    except Exception as e:
        write_new(q/'failure.json',dict(status='SAME_FORWARD_SELECTION_QUEUE_FAILED',error=repr(e),
