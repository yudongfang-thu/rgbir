...
def fixed_frozen_inputs(base):
    root=base/"runs/rgbt_p3_causal_v1/formal_native/dronevehicle"
    return {"teacher":root/"infrared_seed42_native_b32a2/weights/last.pt",
            "reference":root/"rgb_seed42_native_b32a2/weights/last.pt"}
...
def collect_cell(base, seed, arm):
    ...
    try:
        ...
        if launch is not None:
            ...
            for key,expected in fixed_frozen_inputs(base).items():
                require(same_path(launch.get("inputs",{}).get(key,{}).get("path"),expected), f"Launch {key} is not the frozen seed42 baseline")
