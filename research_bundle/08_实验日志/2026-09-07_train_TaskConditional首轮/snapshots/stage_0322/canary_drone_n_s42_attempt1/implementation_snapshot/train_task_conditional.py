...
def build_trainer(cfg, config_path, output, arm, max_steps=None):
    ...
    return legacy.build_trainer(cfg, config_path, output, arm, max_steps)


def validate_config(cfg, arm, formal):
    if arm not in ('n', 'c', 'l', 'cl', 'cgt', 'cl_random'):
    ...
    if cfg['localization_coefficient'] is None and arm not in ('n', 'c'):
...
def run(args):
    ...
    legacy.write_json(args.output / 'launch_manifest.json', {'method_id': cfg['method_id'], 'arm': args.arm,
        ...
        'teacher_labels_used_by_kd': True, 'geometry_contract': cfg.get('geometry_contract'),
        'formal_localization': args.max_steps is None and args.arm not in ('n', 'c')})
    trainer = None
    started = time.time()
    try:
        ...
        if args.max_steps is not None:
            ...
            if args.arm not in ('n', 'c') and not criterion.loc_gradient_checks:
        ...
        legacy.emit_bound_run_receipt(run_dir=args.output/'run_evidence', method_identity=cfg['method_identity'],
            ...
            trainers=[Path(__file__), HERE/'tracked_pair_data.py', *legacy.implementation_files(legacy.DetectionTrainer)],
            losses=[HERE/'task_criterion.py', HERE/'localization_loss.py', LEGACY/'object_evidence_loss.py',
...
def main():
    ...
    p.add_argument('--arm', choices=['n','c','l','cl','cgt','cl_random'], required=True)
