...
def main():
    ...
    roots += [WORKSPACE/'03_现行工程/SpaceNet6_OTD_official_reproduction/experiments/rgbir_task_conditional_v1']
    files=[]
    for root in roots:
        for p in root.rglob('*'):
            if not p.is_file() or p.suffix.lower() not in ALLOWED or '__pycache__' in p.parts:continue
            if p.name=='publication_inventory.json':continue
    ...
    (STAGE/'TASK_CONDITIONAL_BUNDLE_MANIFEST_20260907.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
    (Path(__file__).parent/'publication_inventory.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
    print(json.dumps({'files':len(records),'bytes':sum(r['bytes'] for r in records),'unresolved_links':len(unresolved)}))
