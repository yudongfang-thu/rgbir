...
def projected_gt_check(source, matrix, original_shape, output_size, boxes, classes):
    label_file = Path(img2label_paths([source])[0])
...
def verify_dataset(cfg, vendor_cls, tracked_cls):
    ...
    for index, seed in zip(indices, (0, 42, 123)):
        ...
        for modality, source, matrix, box_key, class_key in (
            ...
            checks[modality] = projected_gt_check(source, matrix, info['original_shape'],
