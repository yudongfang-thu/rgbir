...
def paired_comparison(records, treatment, control):
    ...
    for seed in SEEDS:
        ...
        if any(a.get(field) != b.get(field) for field in ('protocol_id', 'endpoint', 'dataset', 'roster')):
            issues.append(f'seed {seed}: protocol, endpoint, dataset, or roster differs')
            continue
...
def analyze_manifest(manifest, base_dir):
    ...
    for arm in sorted({r['arm'] for r in records}):
        ...
        homogeneous = not valid or all(all(r.get(field) == valid[0].get(field) for field in
                                      ('protocol_id', 'endpoint', 'dataset', 'roster')) for r in valid)
    ...
    return {'schema': 'rgbir-task-conditional-results-v1', 'seed_order': list(SEEDS),
