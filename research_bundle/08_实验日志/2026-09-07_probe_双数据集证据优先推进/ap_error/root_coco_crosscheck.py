...
def main():
    out=HERE/'root_coco_review_v3';out.mkdir(exist_ok=False)
    ...
    with (out/'coco_stdout.log').open('w') as log,contextlib.redirect_stdout(log):
        for e in manifest:
            ...
            default_precision=ev.eval['precision'].copy()
            # TIDE uses integer/100; COCO defaults to np.linspace. Keep both
            # observed outputs to identify floating recall-grid differences.
            ev.params.recThrs=np.arange(101)/100
            ev.accumulate()
            expected=json.loads((results/(e['name']+'.json')).read_text(encoding='utf-8'))
            values={}
            for index,threshold in enumerate((.5,.75)):
                ...
                d=default_precision[index,:,:,0,2];default_ap=float(d[d>-1].mean()*100)
                values[str(threshold)]=dict(coco_AP_with_TIDE_recall_grid=actual,tide_AP=tide,difference_pp=difference,
                    coco_default_AP=default_ap,default_difference_pp=default_ap-tide)
    ...
    summary=dict(status='PASS',independent_evaluator='pycocotools 2.0.7 COCOeval',checks=records,
        matching_scope='AP50/AP75 full caches; maxDets300 all area; no ignore/crowd metadata; both default and TIDE recall grids preserved',
