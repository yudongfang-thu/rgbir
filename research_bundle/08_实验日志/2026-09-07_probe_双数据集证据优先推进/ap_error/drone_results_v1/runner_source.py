...
"""
import argparse, collections, copy, csv, datetime, gzip, hashlib, json, sys, time
...
def audit_input(rows, endpoint):
    ...
    return dict(images=len(rows),gt=total,gt_by_class=dict(counts),predictions=len(scores),
        ...
        gt_population_sha256=hashlib.sha256(json.dumps(canonical,sort_keys=True).encode()).hexdigest(),
        effective_kwargs=contract.get('effective_kwargs'),
        source_files={key:dict(path=endpoint[key],sha256=sha(endpoint[key])) for key in ('path','metric_path','contract_path','receipt_path') if endpoint.get(key)})
...
def main():
    ...
    write_json(out/'execution_environment.json',dict(python=sys.version,executable=sys.executable,numpy=np.__version__,tide_source=json.loads((HERE/'official_source_receipt.json').read_text()),script_sha256=sha(__file__),protocol_sha256=sha(HERE/'PROTOCOL.md'),started_at=datetime.datetime.now().astimezone().isoformat(),cpu_only=True))
    ...
    results={};gt_digests=set()
    for endpoint in endpoints:
        started=time.time();name=endpoint['name'];names=endpoint.get('class_names',NAMES);endpoint['class_names']=names
        rows=read_rows(endpoint['path']);inventory=audit_input(rows,endpoint);gt_digests.add(inventory['gt_population_sha256'])
    ...
    if not args.manifest: assert len(gt_digests)==1,'Six-endpoint GT mismatch'
    write_json(out/'summary.json',dict(status='COMPLETED',same_gt_population=len(gt_digests)==1,results=results))
