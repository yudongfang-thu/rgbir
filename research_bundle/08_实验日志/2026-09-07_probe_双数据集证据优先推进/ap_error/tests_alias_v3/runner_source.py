...
def audit_input(rows, endpoint):
    ...
    if endpoint.get('contract_path'):
        ...
        if 'alias_to_canonical' in contract:
            mapping=contract['alias_to_canonical']
            assert len(mapping)==len(rows) and len(set(mapping.values()))==len(rows)
            assert set(mapping.values())==set(contract['roster'])
            assert all(r['image'] in mapping for r in rows),'Unbound loader alias'
            assert set(mapping[r['image']] for r in rows)==set(contract['roster'])
        else:
        ...
        for key in ('expected_gt','loader_gt','captured_gt'):
            if key in contract: assert contract[key]==total
        if endpoint.get('expected_gt') is not None: assert total==endpoint['expected_gt']>0
    ...
    return dict(images=len(rows),gt=total,gt_by_class=dict(counts),predictions=len(scores),
        ...
        roster_binding='explicit alias_to_canonical bijection' if 'alias_to_canonical' in contract else 'exact stored image path',
        gt_count_bound_to_contract={key:contract[key] for key in ('expected_gt','loader_gt','captured_gt') if key in contract},
...
def validate_synthetic(out):
    ...
    # Mapping contract is authoritative; never silently pair by basename.
    total_gt=sum(len(r['gt_boxes']) for r in rows)
    mapping={r['image']:'/canonical/'+r['image'] for r in rows}
    contract=dict(roster=list(mapping.values()),alias_to_canonical=mapping,expected_val_images=len(rows),
        effective_kwargs=dict(max_det=300,conf=.001),expected_gt=total_gt,loader_gt=total_gt,captured_gt=total_gt)
    path=out/'synthetic_alias_contract.json';write_json(path,contract)
    ep=dict(name='synthetic',path=str(path),contract_path=str(path),class_names=['a','b'],expected_gt=total_gt)
    audit_input(rows,ep)
    invalid=copy.deepcopy(rows);invalid[0]['image']='/unbound/'+invalid[0]['image']
    rejected=False
    try:audit_input(invalid,ep)
    except AssertionError:rejected=True
    assert rejected
    report['alias_binding']=dict(valid_explicit_mapping_accepted=True,unbound_alias_rejected=True,gt_count_checked=True)
