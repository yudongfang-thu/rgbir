...
def main():
    out=HERE/'baseline_class_bridge_v2';out.mkdir(exist_ok=False)
    ...
    dev_images=set(r['image'] for r in allrows if r['split']=='val')
    gt_images=set(r['image'] for r in rows)
    assert len(rows)==3084 and len(dev_images)==200 and len(gt_images)==194
    ...
    old.write_json(out/'summary.json',dict(input=stat(INPUT),source=stat(SOURCE),source_copy_byte_identical=True,images=200,images_with_gt=len(gt_images),images_without_gt=sorted(dev_images-gt_images),settings=dict(confidence=.25,IoU=.5,pair_IoU=.5,view='assigned'),results=results))
