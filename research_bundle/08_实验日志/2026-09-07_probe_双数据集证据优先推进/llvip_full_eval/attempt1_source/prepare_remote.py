...
REMOTE='/mnt/dataset/yudongfang/projects/RGBT_campaign/artifacts/rgbir_evidence_priority_20260907/llvip_full_eval_attempt1'
CODE=r'''
import json,yaml
from pathlib import Path
...
def main():
    ...
    if not a.launch:
        ...
        return
    review=ROOT/'INDEPENDENT_CODE_REVIEW.md'
    ...
    for name in ('export_full_dev.py','run_campaign.py','PROTOCOL.md','spec.json','INDEPENDENT_CODE_REVIEW.md'):
    ...
    with (ROOT/'launch_receipt.json').open('x') as f:json.dump(dict(status='DISPATCHED',remote=REMOTE,screen='evidence_llvip_eval_s42'),f,indent=2)
