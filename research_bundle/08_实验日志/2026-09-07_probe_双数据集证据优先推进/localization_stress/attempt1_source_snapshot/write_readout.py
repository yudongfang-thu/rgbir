...
lines = ['# 固定 anchor 定位目标平移压力诊断（2026-09-07）','',
...
for r in worst:
    if r['split']=='val' and r['fixed_stratum']=='quality_gate070':
        lines.append(f"|{display[r['dataset']]}|{r['linf_px']}|{r['fixed_n']}|{v(r,'min_iou_advantage_mean'):+.5f}|{v(r,'max_teacher_minus_native_ce_mean'):+.5f}|{pct(r,'all_directions_iou_better_fraction')}|{pct(r,'all_directions_ce_better_fraction')}|{pct(r,'all_directions_positive_cosine_fraction')}|{r['all_directions_gate_n']}/{r['fixed_n']}|")
