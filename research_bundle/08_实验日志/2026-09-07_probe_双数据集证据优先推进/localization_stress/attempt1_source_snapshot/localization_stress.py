        ...
        for split in ('train','val'):
            for name, f in fixed.items():
                ...
                row = {'dataset':dataset,'split':split,'fixed_stratum':name,'linf_px':amplitude,'directions':8,'fixed_n':count,
                       'all_directions_gate_n':int((m & worst['all_directions_gate']).sum()),
                       'all_directions_gate_fraction':float(worst['all_directions_gate'][m].mean()) if count else None,
                ...
                for key,val in worst.items(): add_metric(row,key,val,m)
...
def main():
    ...
    parser.add_argument('--out', type=Path, default=HERE/'outputs_attempt1')
