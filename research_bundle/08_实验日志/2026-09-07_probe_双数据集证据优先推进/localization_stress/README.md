...
> **已执行两数据集的固定 25 条件 CPU 诊断，attempt2已获独立审阅限定描述性PASS（见independent_review/EXPERIMENT_AUDIT.md）。LLVIP 的条件定位质量在本压力测试中比 Drone 更稳定；其 dev 固定质量门的 61 对象在四输入像素八方向下仍有 55/61 的教师 DFL CE 始终更低，但 gate 仅 34/61 全方向存活，训练快照只有 23 个门内对象。此结论不能替代原 L1 准入或实际定位训练。**
...
- [精简逐条件原始表](outputs_attempt2/compact_metrics.csv)、[完整分布统计](outputs_attempt2/all_metrics.csv)、[八方向最差原始表](outputs_attempt2/all_worst_case.csv)。
...
- [CPU真值回执](outputs_attempt2/known_truth_tests.json)：11项通过；64个KD与GT梯度均以中心有限差分核验，另含质量守恒/无clamp/零扰动exact/严格.70/固定mask等。
...
- [完成回执](outputs_attempt2/completion_receipt.json)：`COMPLETED_PENDING_INDEPENDENT_REVIEW`。
