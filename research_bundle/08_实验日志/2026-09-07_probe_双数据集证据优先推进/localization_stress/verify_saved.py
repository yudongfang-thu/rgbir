...
for dataset in ('llvip','dronevehicle'):
    ...
    with (OUT/dataset/'worst_case.csv').open(encoding='utf-8-sig',newline='') as handle:
        worst_rows=list(csv.DictReader(handle))
    for r in worst_rows:
        m=a['fixed_'+r['fixed_stratum']] & (a['split']==r['split'])
        amp=int(r['linf_px'])
        for kind in ('gate','teacher_support'):
            truth=a['worst_'+str(amp)+'_all_directions_'+kind]
            assert int((m & truth).sum())==int(r['all_directions_'+kind+'_success_n'])
            assert int(m.sum())==int(r['all_directions_'+kind+'_denominator_n'])
            assert 'all_directions_'+kind+'_n' not in r
    with np.load(HERE/'outputs_attempt1'/dataset/'per_object.npz',allow_pickle=False) as old:
        assert set(old.files)==set(a)
        for k in a:
            if np.issubdtype(a[k].dtype,np.inexact): np.testing.assert_array_equal(a[k],old[k])
            else: assert np.array_equal(a[k],old[k]),k
    checks[dataset]={'status':'PASSED','prior_saved_zero_rowwise_max_abs':deltas,'all_saved_table_reducer_max_abs':max_summary_error,
                     ...
                     'worst_success_and_denominator_counts_from_saved_arrays_exact':True,'all_per_object_arrays_exact_vs_attempt1':True}
