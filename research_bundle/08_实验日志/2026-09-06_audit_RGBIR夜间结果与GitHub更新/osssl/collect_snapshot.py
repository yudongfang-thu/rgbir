...
for subroot in (root/'runs/osssl_ir_20260906', root/'runs/cgkd_w1'):
    for run in sorted(subroot.iterdir()):
        ...
        # Inventory named evaluation artifacts recursively; no checkpoint loading.
        item['evaluation_candidates'] = []
        for p in sorted(run.rglob('*')):
            if p.is_file() and p.suffix in ('.json', '.csv', '.yaml', '.txt') and any(t in str(p.relative_to(run)).lower() for t in ('eval', 'metric', 'receipt')):
                item['evaluation_candidates'].append({'relative':str(p.relative_to(run)),'bytes':p.stat().st_size})
...
processes = '\n'.join(line for line in ps.splitlines() if 'osssl' in line and any(t in line for t in ('worker', 'project_resource_guard.py', 'eval_rgbt_detector.py', 'train_native_rgbt.py')))
