...
OUT = BASE / 'artifacts/audit_rgbir_night_20260906_2144'
...
for cell in endpoint['cells']:
    ...
    if cell['status'] == 'completed':
        for evidence_dir in ['run_evidence', 'eval_evidence']:
            for p in (run/evidence_dir).rglob('*'):
                if p.is_file():
                    copy_small(p, dest/p.relative_to(run))
        copy_small(run/'evaluation_val_roster.txt', dest/'evaluation_val_roster.txt')
    ...
    rows = [{k.strip(): v for k,v in r.items() if k is not None and v is not None} for r in parsed if all(r.get(k) is not None for k in ['epoch','time','train/box_loss','train/cls_loss','train/dfl_loss'])]
    item['ignored_incomplete_csv_rows'] = len(parsed)-len(rows)
    item['ragged_csv_rows'] = sum(any(v is None for v in r.values()) for r in parsed)
    item['csv_note'] = 'Only epoch/time/train-loss fields are interpreted. Final validate() may remove metric columns; AP is never read from this CSV.'
