"""Reuse N receipt checks for C0; add actual weighted-KD scalar closure; decimal 300 GB limit."""
...
t = read(B/'runs/C0/short_training_receipt.json')
e = read(B/'evaluations/C0/short_evaluation_receipt.json')
c = read(B/'evaluations/C0/short_evaluation_contract.json')
r = read(B/'runs/C0/runtime_ready.json')
cfg = load_yaml(B/'runs/C0/short_screen_config.yaml')
args = load_yaml(B/'runs/C0/args.yaml')
frozen = load_yaml(STAGE/'launch_evidence_attempt1/source/configs/drone_C0_s42_E8.yaml')
accepted = read(NATIVE/'evidence_contract.json')
binding = read(NATIVE/'evaluation_profile_binding/binding.json')
rows = list(csv.reader((B/'runs/C0/results.csv').read_text().splitlines()))
kd = [json.loads(x) for x in (B/'runs/C0/kd_batches.jsonl').read_text().splitlines() if x]
checks = {}
...
weighted_errors = [abs(x['weighted_kd_total']-x['loss_unweighted']*len(x['student_files'])*.1) for x in kd]
total_errors = [abs(x['total_loss']-f32(x['native_total']+f32(x['loss_unweighted']*f32(len(x['student_files'])*.1)))) for x in kd]
checks['actual_c0_learning_coefficient'] = cfg['classification_coefficient']==t['classification_coefficient']==.1 and cfg['localization_coefficient']==t['localization_coefficient']==0 and all(x['arm']=='paired' and x['kd_weight']==.1 and x['weighted_kd_total']>0 for x in kd)
...
checks['short_identity'] = e['status']=='SHORT_SCREEN_EVALUATION_COMPLETED' and all(x['scope']=='SHORT_SCREEN' and x['arm']=='C0' and x['seed']==42 and x['single_seed'] is True and x['formal_e200_complete'] is False and x['formal_paper_gain_claim'] is False and x['official_test_accessed'] is False for x in (t,e))
roster = (B/'evaluations/C0/development_roster.txt').read_text().splitlines()
...
for phase, receipt in [('train',t),('eval',e)]:
    p = read(B/f'queue/short_C0_s42_E8_{phase}_resource_profile.json')
    status = read(B/f'queue/short_C0_s42_E8_{phase}_status.json')
    done = read(B/f'queue/short_C0_s42_E8_{phase}_completed.json')
    ...
    v=resource[phase]
    checks[phase+'_queue_resources'] = p['status']==status['status']=='COMPLETED' and p['exit_code']==status['exit_code']==0 and not p['monitor_errors'] and not status['monitor_errors'] and done['status']=='SHORT_SCREEN_STAGE_COMPLETED' and done['step']['arm']=='C0' and done['step']['stage']==phase and v['minimum_free_mib']==p['minimum_free_mib']>=2048 and v['max_project_rss_mib']*2**20<=300*10**9 and v['peak_job_vram_mib']<=p['launch']['expected_vram_mib'] and v['peak_job_rss_mib']<=p['launch']['expected_rss_mib']
    manifest=read(B/('runs/C0/source_manifest.json' if phase=='train' else 'evaluations/C0/source_manifest.json'))
    checks[phase+'_source_receipt_byte_identity'] = all(x['byte_identity'] is True for x in manifest['files'])
result = dict(status='PASS_RECEIPT_CROSSCHECK_SINGLE_SEED_E8_C0' if all(checks.values()) else 'FAIL',checks=checks,metric_means=means,metric_percent={k:e[k]*100 for k in means},resources=resource,training_counters={k:t[k] for k in ('batches','optimizer_updates','attempts','amp_skips','ema_updates')},checkpoint_stat=t['checkpoint'],csv_header_columns=len(rows[0]),csv_row_columns=[len(x) for x in rows[1:]],logged_kd_rows=len(kd),weighted_kd_double_maxabs=max(weighted_errors),total_loss_float32_maxabs=max(total_errors),project_rss_limit_decimal_bytes=300*10**9,limitations=['Training CSV final row is short; do not read AP by its header. Independent evaluation receipt is the AP source.','Population and checkpoint stat crosscheck, not per-image GT byte audit or checkpoint content equality.','Counter closure uses executed receipts and sparse logs; no independent replay of all optimizer steps.','No inference, GPU, weights/state loading, new hashes, or gain judgment.'])
