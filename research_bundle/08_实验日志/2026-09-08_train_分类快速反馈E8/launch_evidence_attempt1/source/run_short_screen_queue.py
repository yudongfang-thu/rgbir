...
Ready for root admission and invocation; this entry does not create admission.
"""
import argparse
import math
...
def jobs_for(args):
    jobs=[]
    for arm in ARMS:
        config=args.config_dir/('drone_'+arm+'_s42_E8.yaml')
        admission=args.admissions/(arm+'.json')
        run=args.campaign_root/'runs'/arm
        evaluation=args.campaign_root/'evaluations'/arm
        jobs.append(dict(id='short_'+arm+'_s42_E8_train',arm=arm,stage='train',kind='train',formal=False,
        ...
        jobs.append(dict(id='short_'+arm+'_s42_E8_eval',arm=arm,stage='eval',kind='eval',
...
def check_terminal(job,receipt):
    ...
    if job['stage']=='train':
        if receipt.get('epochs_configured')!=8 or receipt.get('last_epoch')!=8:raise ValueError('Not complete E8 training')
    elif receipt.get('epochs')!=8 or receipt.get('full_dev_images')!=1469 or receipt.get('full_dev_gt_objects')!=22462:
...
def run(args):
    ...
    config_records=[]
    for arm in ARMS:
        config=args.config_dir/('drone_'+arm+'_s42_E8.yaml');cfg=load_config(config)
    ...
    write_new(queue/'manifest.json',dict(scope='SHORT_SCREEN_SINGLE_SEED_E8',jobs=jobs,configs=config_records,
        ...
        independent_lr_horizon=8,seed=42,sequence=['N train','N eval','C0 train','C0 eval','C1 train','C1 eval'],
    ...
    try:
        ...
        write_new(queue/'completion.json',dict(status='SHORT_SCREEN_MATRIX_COMPLETED',completed=completed,
            seconds=time.time()-started,single_seed=True,epochs=8,new_hash_computed=False,
