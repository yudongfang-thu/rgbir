...
> **21:46更新**：请先读[最新结果与证据](LATEST_RESULTS.md)。OEv1已有3个完整端点但0个同seed完整配对；OS-SSL首次同seedP−shuffled CSV为+0.669mAP/+0.495AP50，尚非三seed归因。新增的native初始化混杂、RGB-only控制缺口和OEv1终态CSV列错位都须纳入审查。
