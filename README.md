> **当前统一进度（2026-09-09）**：[数据／复现／自研方法三线总整理](research_bundle/07_研究分析/项目三线进度与证据总整理_20260909.md)。新增实验与自动扩展已暂停，94保留两项已有C1，90没有原协议重训在跑。C0有三seed小幅信号，尚无完成四臂归因的原创方法。下方均为各日期历史快照。

> **C1预算门与重启（2026-09-09 17:58）**：[运行证据](research_bundle/08_实验日志/2026-09-09_train_C1十小时完整训练/recovery_20260909_1755/README.md)。seed42稳定约130秒/轮、含1小时余量外推约8.36h；seed0共享卡第6轮预算中止，原配置新attempt已在空GPU6启动。当前两项运行，原失败证据保留，无新AP。

> **C1完整训练已启动（2026-09-09 16:46）**：[执行与运行证据](research_bundle/08_实验日志/2026-09-09_train_C1十小时完整训练/README.md)。94动态分配GPU0，Drone全量C1 seed42 E200；自动独立dev评价与单run十小时预算已接通。7项运行测试和三seed配置预检通过，0/123按首5轮工程门自动接续；尚无新完整AP。下方暂停/未启动为历史时点。

> **十小时首发建议（2026-09-09）**：[首个完整实验的选择与执行缺口](research_bundle/08_实验日志/2026-09-09_audit_十小时首发实验选择/README.md)。优先Drone C1加速版seed42完整E200；已有7.9倍提速，6.92小时纯训练仍为短窗外推。补自动dev评价及run级10小时限时，在首个正式run内核对整轮速度。本轮仅核对与建议，未启动GPU。

> **94资源更新（2026-09-09 15:07）**：[恢复后资源复核](research_bundle/08_实验日志/2026-09-09_ops_94恢复后资源复核/README.md)。仍识别8张4090，0–3四张空卡、4–7他人使用，可以按实时资源继续实验。当前无本项目训练进程；不占满四张空卡，旧慢版任务未自动重启。下方13:06“全部GPU占用”是旧快照。
...
> **2026-09-08 子集E8筛选器校验：本阶段已完成**：N/C0子集E8两臂及完整dev实测9分42秒；mAP27.588046/26.459942，差−1.128105pp；本筛选器未保留已知方向，结束版本。 原失败attempt与独立审阅保留；当前状态和证据边界见[条目README](research_bundle/08_实验日志/2026-09-08_probe_子集筛选器校验/README.md)。

> **2026-09-08 对象坐标DFL短筛：本阶段已完成**：三臂短筛11分6.86秒完成；DFL未胜N与GT，结束当前L3微调版本。 原失败attempt与独立审阅保留；当前状态和证据边界见[条目README](research_bundle/08_实验日志/2026-09-08_probe_对象坐标分布目标/README.md)。
